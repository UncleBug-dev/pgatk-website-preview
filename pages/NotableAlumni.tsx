  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-20 font-sans">
      ...
      <div className="bg-primary-900 text-white pt-10 pb-20 md:pt-14 md:pb-24 relative overflow-hidden">
        ...
        <div className="w-full max-w-[1600px] mx-auto px-4 md:px-8 relative z-10">
          ...
          <nav className="flex flex-wrap items-center gap-2 text-sm text-slate-300 mb-6 font-medium">
            ...
            <ChevronRight className="w-3 h-3 opacity-40" />
            <span className="text-accent-500 font-bold truncate">Выдающиеся выпускники</span>
          </nav>
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-slate-100 p-6 md:p-10 relative overflow-hidden">
              ...
              <div className="prose prose-slate max-w-none mb-10 relative z-10">
                <p className="text-lg md:text-xl text-slate-700 leading-relaxed font-light">
                  Мы гордимся каждым нашим выпускником. Многие из них добились значительных успехов в профессиональной деятельности, внесли весомый вклад в развитие агропромышленного комплекса и других сфер жизни нашей страны. 
                  В этой галерее представлены фотографии некоторых из наших выдающихся выпускников, чьи достижения являются отличным примером для нынешних учащихся.
                </p>
              </div>

              {/* Photo Gallery Grid */}
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 md:gap-6 relative z-10">
                {ALUMNI_IMAGES.map((img, index) => {
                  const imagePath = `${import.meta.env.BASE_URL}images/VidayshiesyVipusnkiki/${img}`;
                  return (
                    <div 
                      key={index}
                      className="group relative aspect-[3/4] rounded-xl overflow-hidden cursor-pointer shadow-sm hover:shadow-xl transition-all duration-500 hover:-translate-y-1"
                      onClick={() => setSelectedImageIndex(index)}
                    >
                      <img 
                        src={imagePath} 
                        alt={`Выдающийся выпускник ${index + 1}`}
                      ...
                      <div className="absolute inset-0 bg-gradient-to-t from-primary-900/80 via-primary-900/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4">
                        <div className="transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300 flex items-center gap-2">
                           ...
                           <span className="text-white text-xs font-bold uppercase tracking-wider">Увеличить</span>
                        </div>
                      </div>
                    </div>
                  );
      ...
      {selectedImageIndex !== null && (
        <Lightbox 
          images={ALUMNI_IMAGES.map(img => `${import.meta.env.BASE_URL}images/VidayshiesyVipusnkiki/${img}`)}
