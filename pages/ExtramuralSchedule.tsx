...
const ExtramuralSchedule: React.FC = () => {
  ...
  return (
    <div className="animate-in fade-in duration-500 w-full space-y-10">
      ...
      {/* Intro section */}
      <div className="bg-gradient-to-r from-blue-500 to-indigo-700 rounded-3xl p-8 md:p-10 text-white shadow-xl relative overflow-hidden group">
        ...
        <div className="relative z-10">
          ...
          <h2 className="text-3xl md:text-4xl font-display font-bold mb-4">График образовательного процесса</h2>
          <p className="text-lg text-blue-100 max-w-2xl leading-relaxed">
            Ознакомьтесь с актуальным графиком образовательного процесса для учащихся заочного отделения.
          </p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {images.map((img, index) => (
            <div 
              ...
              className="cursor-pointer group relative rounded-xl overflow-hidden shadow-sm hover:shadow-xl border border-slate-100 bg-slate-50"
            >
              <img src={img} alt={`График ${index + 1}`} className="w-full object-contain group-hover:scale-[1.02] transition-transform duration-500" />
              <div className="absolute inset-0 bg-gradient-to-t from-primary-900/40 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6">
                 <span className="text-white font-bold drop-shadow-md">Нажмите, чтобы увеличить</span>
              </div>
            </div>
          ))}
        </div>
...
export default ExtramuralSchedule;
