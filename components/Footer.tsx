...
const Footer: React.FC = () => {
  ...
  return (
    <footer>
      ...
      <div className="bg-slate-900 text-slate-300 pt-8 pb-6 md:pt-16 md:pb-8 border-t-4 border-accent-500 text-sm">
        <div className="w-full max-w-[1600px] mx-auto px-4 md:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12 mb-8 md:mb-12">
            ...
            <div>
              <div className="bg-slate-800/80 rounded-xl border border-slate-700 mb-6 p-4 text-xs md:text-sm">
                <p className="mb-2"><strong>Официальный сайт</strong><br className="hidden sm:block"/>Учреждения образования «Пинский государственный аграрно-технический колледж имени А.Е.Клещева» зарегистрирован в Государственном регистре информационных ресурсов Беларуси.</p>
                <a 
                  href={resolvePath('downloads/footer/img601.webp')} 
                  target="_blank" 
                  rel="noopener noreferrer" 
                  className="flex items-center text-accent-500 hover:text-white transition-colors mt-2"
                >
                  <ExternalLink className="w-4 h-4 mr-2 flex-shrink-0" />
                  <span>Регистрационное свидетельство №2142335835 от 21.09.2023</span>
                </a>
              </div>
              
              <h3 className="text-white font-display font-bold text-base md:text-lg mb-3 md:mb-4 mt-2 md:mt-6">Полезные ссылки</h3>
              <ul className="space-y-2 text-xs md:text-sm flex flex-col">
            ...
            <div>
              <h3 className="text-white font-display font-bold text-base md:text-lg mb-4 md:mb-6">Наш адрес и контакты</h3>
              <div className="space-y-3 md:space-y-4 text-xs md:text-sm">
                <p className="flex items-start space-x-3">
                  <MapPin className="w-4 h-4 md:w-5 md:h-5 text-accent-500 flex-shrink-0 mt-0.5" />
                  <span><AddressLink>{settings.address}</AddressLink></span>
                </p>
                <p className="flex items-center space-x-3">
                  <Phone className="w-4 h-4 md:w-5 md:h-5 text-accent-500 flex-shrink-0" />
                  <span>Приемная (факс): {settings.phone}<br/>Директор: 8 (0165) 63-94-99</span>
                </p>
                <p className="flex items-center space-x-3">
                  <Mail className="w-4 h-4 md:w-5 md:h-5 text-accent-500 flex-shrink-0" />
                  <a href={`mailto:${settings.email}`} className="hover:text-white transition-colors">{settings.email}</a>
                </p>
              </div>

              <div className="mt-4 md:mt-6 pt-4 md:pt-6 border-t border-slate-800 text-xs md:text-sm">
                ...
                <p className="text-[10px] md:text-xs text-slate-500 mt-1">(Форма обращения к администратору веб-сайта)</p>
              </div>

              {/* Social Networks Placeholder */}
              <div className="mt-4 md:mt-6">
                <p className="mb-2 md:mb-3 text-white font-bold text-xs md:text-sm">Официальные социальные сети:</p>
                <div className="flex flex-wrap gap-2 md:gap-3">
                  <a href="https://vk.com/pgatkk" target="_blank" rel="noopener noreferrer" className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-slate-800 hover:bg-[#0077FF] flex items-center justify-center transition-colors text-slate-300 hover:text-white group" aria-label="VK">
                  ...
                  <a href="https://www.instagram.com/pgatkk" target="_blank" rel="noopener noreferrer" className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-slate-800 hover:bg-gradient-to-tr hover:from-yellow-400 hover:via-pink-500 hover:to-purple-500 flex items-center justify-center transition-all text-slate-300 hover:text-white group" aria-label="Instagram">
                    <svg className="w-4 h-4 md:w-5 md:h-5 fill-current group-hover:scale-110 transition-transform" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>
                  </a>
                  <a href="https://t.me/pgatkk" target="_blank" rel="noopener noreferrer" className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-slate-800 hover:bg-[#229ED9] flex items-center justify-center transition-colors text-slate-300 hover:text-white group" aria-label="Telegram">
                    <svg className="w-4 h-4 md:w-5 md:h-5 fill-current group-hover:scale-110 transition-transform" viewBox="0 0 24 24"><path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.894 8.221l-1.97 9.28c-.145.658-.537.818-1.084.508l-3-2.21-1.446 1.394c-.14.18-.357.223-.548.223l.188-2.85 5.181-4.686c.223-.198-.054-.31-.346-.11l-6.4 4.024-2.76-.86c-.6-.185-.615-.6.125-.89l10.793-4.159c.5-.185.951.106.78.823z"/></svg>
                  </a>
                  <a href="https://www.tiktok.com/@_pgatkk" target="_blank" rel="noopener noreferrer" className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-slate-800 hover:bg-black hover:border hover:border-slate-700 flex items-center justify-center transition-colors text-slate-300 hover:text-white group" aria-label="TikTok">
                    <svg className="w-4 h-4 md:w-5 md:h-5 fill-current group-hover:scale-110 transition-transform" viewBox="0 0 24 24"><path d="M12.525.02c1.31-.02 2.61-.01 3.91.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.18-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.12-3.44-3.17-3.64-5.46-.22-2.5.83-5.06 2.74-6.66 1.72-1.46 4.14-2.02 6.32-1.49v4.22c-1.05-.18-2.12-.17-3.11.13-1.02.32-1.87 1.02-2.39 1.93-.65 1.14-.65 2.65.01 3.79.67 1.16 2.01 1.83 3.36 1.76 1.28-.06 2.45-.73 3.08-1.83.61-1.07.82-2.3.82-3.55.01-4.48.01-8.96 0-13.44zm-7.66 6.07c.04-.02.08-.04.12-.06-.04.02-.08.04-.12.06z"/></svg>
                  </a>
                  <a href="https://youtube.com/channel/UCx3boiuvaRX1PA-yEXi5hZw" target="_blank" rel="noopener noreferrer" className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-slate-800 hover:bg-[#FF0000] flex items-center justify-center transition-colors text-slate-300 hover:text-white group" aria-label="YouTube">
            ...
            <div>
              <h3 className="text-white font-display font-bold text-base md:text-lg mb-4 md:mb-6">Условия использования сайта</h3>
              <p className="text-[11px] md:text-xs text-slate-400 leading-relaxed mb-4 md:mb-6 text-justify">
          ...
          <div className="border-t border-slate-800 pt-6 flex flex-col md:flex-row justify-between items-center gap-4 text-[10px] md:text-xs text-slate-500 text-center md:text-left">
