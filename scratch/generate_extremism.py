...
def main():
    ...
    output_file = r'd:\Workspace\Web\PGATK Website\pages\ExtremismPrevention.tsx'
...
export default function ExtremismPrevention() {{
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-20 font-sans pt-12">
      {{/* Hero Section */}}
      <div className="relative bg-gradient-to-br from-slate-900 via-slate-800 to-red-900 text-white overflow-hidden py-24 mb-12 rounded-3xl mx-4 lg:mx-8 shadow-2xl">
        ...
        <div className="container mx-auto px-6 relative z-10 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 backdrop-blur-md border border-white/20 mb-6">
            ...
            <span className="text-red-100 font-medium">Безопасность и Закон</span>
          </div>
          ...
          <p className="text-xl md:text-2xl text-slate-300 mb-8 max-w-3xl mx-auto font-medium">
            Законодательство Республики Беларусь о противодействии экстремизму и ответственность за правонарушения
          </p>
        </div>
      </div>

      <div className="container mx-auto px-4 lg:px-8 max-w-5xl" onClick={{handleClick}}>
        <div className="bg-white rounded-3xl shadow-xl border border-slate-200 p-8 md:p-12 mb-8 prose prose-lg max-w-none text-slate-700 prose-headings:text-slate-800 prose-a:text-red-600 hover:prose-a:text-red-800" dangerouslySetInnerHTML={{{{ __html: `{intro_html}` }}}} />
      </div>
