...
def main():
    input_file = r'd:\Workspace\Web\PGATK Website\Родителям.txt'
    output_file = r'd:\Workspace\Web\PGATK Website\pages\Parents.tsx'
    ...
    # Extract tabs from HTML
    tabs_matches = re.finditer(r'<dd class="tabs"[^>]*>(.*?)</dd>', html_content, flags=re.DOTALL)
    tabs_data = []
    
    for idx, match in enumerate(tabs_matches):
        tab_content = match.group(1).strip()
        tab_content = tab_content.replace('`', '\\`').replace('$', '\\$')
        
        title = f"Вкладка {idx+1}"
        title_match = re.search(r'<p class="text-center mb-4 text-slate-700 leading-relaxed">\s*<strong>(.*?)</(?:strong|span)>', tab_content)
        if title_match:
    ...
    # Generate React component with Tabs
    react_code = f"""import React, {{ useState }} from 'react';
import {{ HeartHandshake, ChevronRight, BookOpen }} from 'lucide-react';
...
export default function Parents() {{
  ...
  const tabs = [
    {','.join([f"{{ id: '{t['id']}', title: '{t['title']}', content: `{t['content']}` }}" for t in tabs_data])}
  ];
    ...
    <div className="bg-slate-50 min-h-screen pb-20 font-sans pt-12">
      ...
      <div className="relative bg-gradient-to-br from-indigo-900 via-blue-900 to-slate-900 text-white overflow-hidden py-24 mb-12 rounded-3xl mx-4 lg:mx-8 shadow-2xl">
        <div className="absolute inset-0 opacity-20">
          <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1511895426328-dc8714191300?ixlib=rb-4.0.3&auto=format&fit=crop&w=2000&q=80')] bg-cover bg-center mix-blend-overlay"></div>
        </div>
        
        <div className="container mx-auto px-6 relative z-10 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 backdrop-blur-md border border-white/20 mb-6">
            <HeartHandshake className="w-5 h-5 text-blue-300" />
            <span className="text-blue-100 font-medium">Для родителей и законных представителей</span>
          </div>
          ...
          <p className="text-xl md:text-2xl text-blue-200 mb-8 max-w-3xl mx-auto font-medium">
            Советы психологов, правила безопасности и полезные материалы для воспитания
          </p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 lg:px-8 max-w-6xl">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          {{/* Tab Content */}}
          <div className="w-full lg:w-2/3" onClick={{handleClick}}>
